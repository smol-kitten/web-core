...
echo "Configuring Apache + PHP-FPM | QUICK_SET=${QUICK_SET:-default}"
...
# Security headers & HSTS via site conf (mod_headers must be enabled)
if [ -f "$SITE_CONF" ]; then
    if [ "$SECURITY_HEADERS" = "false" ]; then
        sed -i '/Header always set X-/d'           "$SITE_CONF"
        sed -i '/Header always set Referrer/d'     "$SITE_CONF"
        sed -i '/Header always set Permissions/d'  "$SITE_CONF"
    fi
    if [ "$FORCE_HTTPS" = "true" ]; then
        grep -q "Strict-Transport-Security" "$SITE_CONF" || \
            sed -i '/<\/VirtualHost>/i\    Header always set Strict-Transport-Security "max-age=31536000; includeSubDomains; preload"' "$SITE_CONF"
    fi
    if [ -n "$CSP_HEADER" ]; then
        grep -q "Content-Security-Policy" "$SITE_CONF" || \
            sed -i '/<\/VirtualHost>/i\    Header always set Content-Security-Policy "'"${CSP_HEADER}"'"' "$SITE_CONF"
    fi
fi
