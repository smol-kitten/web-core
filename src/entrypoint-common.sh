...
configure_security_php() {
    ...
    # Hide PHP version
    sed -i "s/^expose_php = .*/expose_php = ${PHP_EXPOSE:-Off}/" "$ini"

    # Strict session mode
    if grep -q '^;*session.use_strict_mode' "$ini"; then
        sed -i "s/^;*session.use_strict_mode = .*/session.use_strict_mode = 1/" "$ini"
    else
        echo "session.use_strict_mode = 1" >> "$ini"
    fi

    # Disable dangerous PHP functions when opted in
    if [ "${PHP_DISABLE_DANGEROUS_FUNCTIONS:-false}" = "true" ]; then
        local DANGER="exec,passthru,shell_exec,system,proc_open,popen,pclose,pcntl_exec,dl"
        if [ -n "${DISABLE_FUNCTIONS:-}" ]; then
            # Merge with user-provided list
            DANGER="${DISABLE_FUNCTIONS},${DANGER}"
        fi
        sed -i "s/^disable_functions = .*/disable_functions = ${DANGER}/" "$ini"
    fi
}
...
# -----------------------------------------------------------------------
# Background daemons (health worker + telemetry)
# -----------------------------------------------------------------------
start_background_workers() {
    if [ "${HEALTH_WORKER:-true}" = "true" ]; then
        echo "Starting health worker (interval: ${HEALTH_WORKER_INTERVAL:-86400}s)..."
        /docker-health-worker.sh &
    fi

    if [ "${OTEL_ENABLED:-false}" = "true" ]; then
        echo "Starting OpenTelemetry exporter → ${OTEL_ENDPOINT:-http://localhost:4318/v1/metrics}"
        /docker-telemetry.sh &
    fi
}
